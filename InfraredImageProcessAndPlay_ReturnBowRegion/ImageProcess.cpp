	...
	else
	{
		try
		{
			if(IsRecByTrainer == false) 
			{
				...
				//imshow("srcMat", image_Para.src_Mat);
				//waitKey(0);
...
int Height_Stagger::ConfirmBowPosition(int& HL)   //定位出弓的位置
{
	...
	try
	{
		...
		//imshow("roiBinary",image_Para.input_PreProcessROIBinMat);
		//waitKey(0);
		...
		//for(int row = 0;row<image_Para.input_PreProcessROIBinMat.rows;row++)   //右边界10列的255点数
		//{
		//	int R_edge_num = 0;
		...
		if(HL==-1 || HL<image_Para.input_PreProcessROIBinMat.rows/10 || HL>image_Para.input_PreProcessROIBinMat.rows*9/10)
		{
			...
			//imshow("canny", image_Para.input_PreProcessROICannyMat);
			//waitKey(0);
...
bool Height_Stagger::DetectBowRectRegion(Mat& src_gray_Mat,vector<Rect>& rects)
{
	...
	image_Para.cascade.detectMultiScale(src_gray_Mat,rectBlock,1.05, 6, 0);
	if(rectBlock.size()>0)
	{
		return true;
	}
	...
	{
		image_Para.cascade.detectMultiScale(src_gray_Mat,rectBlock,1.05, 5, 0);
		if(rectBlock.size()>0)
		{
			return true;
		}
...
bool Height_Stagger::DrawRecognizeResult(Mat resultMat,int recFrame)
{
	...
	double x_Scale = 1;  //(rect.right - rect.left) / (double)resultMat.cols;
	double y_Scale = 1;  //(rect.bottom - rect.top) / (double)resultMat.rows;
	...
	//{
	//	line(resultMat, Point(0, (int)AlarmHeightPixel*y_Scale), Point((int)319 *  ScaleFactor*  x_Scale, (int)AlarmHeightPixel*y_Scale), Scalar(255, 0, 255), 1, 8);  //紫色线条标记出导高位置
	//
	//}
	...
	else
	{
		...
		for (int t = 0; t<BowNet_Result.net_Lines.size(); t++)
		{
			...
			if (t == ObliqueNumber)
			{
				line(resultMat, Point(BowNet_Result.net_Lines.at(t).x1*x_Scale * ScaleFactor, BowNet_Result.net_Lines.at(t).y1*y_Scale * ScaleFactor), Point(BowNet_Result.net_Lines.at(t).x2*x_Scale * ScaleFactor, BowNet_Result.net_Lines.at(t).y2*y_Scale * ScaleFactor), CV_RGB(255, 0, 0), 1, 8);
			}
			...
			else
			{
				...
				{
					line(resultMat, Point(BowNet_Result.net_Lines.at(t).x1*x_Scale* ScaleFactor, BowNet_Result.net_Lines.at(t).y1*y_Scale* ScaleFactor), Point(BowNet_Result.net_Lines.at(t).x2*x_Scale* ScaleFactor, BowNet_Result.net_Lines.at(t).y2*y_Scale* ScaleFactor), color, 1, 8);
				}
			}
		}
		...
		{
			line(resultMat, Point(0, (int)(240 - BowNet_Result.pixel_BowHeight / ScaleFactor)*y_Scale*  ScaleFactor), Point((int)319 * x_Scale*  ScaleFactor, (int)(240 - BowNet_Result.pixel_BowHeight / ScaleFactor)*y_Scale*  ScaleFactor), CV_RGB(255, 0, 255), 1, 8);  //标记出导高位置
			line(resultMat, Point((int)(recResult.midPoint.x*  ScaleFactor - PullOutPixelSpan)*x_Scale, (int)(240 - BowNet_Result.pixel_BowHeight / ScaleFactor + 5)*y_Scale*  ScaleFactor),
				Point((int)(recResult.midPoint.x *  ScaleFactor + PullOutPixelSpan)*x_Scale, (int)(240 - BowNet_Result.pixel_BowHeight / ScaleFactor + 5)*y_Scale*  ScaleFactor), Scalar(255, 255, 0), 1, 8);   //画拉出值的水平轴刻度线
			...
			{
				itoa(i, str, 10);
				putText(resultMat, str, Point((int)(t - 10 * ScaleFactor)*x_Scale, (int)(240 - BowNet_Result.pixel_BowHeight / ScaleFactor + 25)*y_Scale*  ScaleFactor), FONT_HERSHEY_PLAIN, 0.5*ScaleFactor, Scalar(255, 0, 255), 1, 8);
				line(resultMat, Point((int)t*x_Scale, (int)(240 - BowNet_Result.pixel_BowHeight / ScaleFactor + 5)*y_Scale*  ScaleFactor), Point((int)t*x_Scale, (int)(240 - BowNet_Result.pixel_BowHeight / ScaleFactor + 15)*y_Scale*  ScaleFactor), Scalar(255, 255, 0), 1, 8);  //画出拉出值为负的刻度线
			}

			itoa(0, str, 10);
			putText(resultMat, str, Point((int)(BowNet_Result.midPx*  ScaleFactor - 3)*x_Scale, (int)(240 - BowNet_Result.pixel_BowHeight / ScaleFactor + 33)*  ScaleFactor*y_Scale), FONT_HERSHEY_PLAIN, 0.5*ScaleFactor, Scalar(255, 0, 255), 1, 8);	 //在受电弓的中点列标记出0值
			line(resultMat, Point((int)(BowNet_Result.midPx*x_Scale*  ScaleFactor), (int)(240 - BowNet_Result.pixel_BowHeight / ScaleFactor + 5)*y_Scale*  ScaleFactor),
				Point((int)(BowNet_Result.midPx*x_Scale*  ScaleFactor), (int)(240 - BowNet_Result.pixel_BowHeight / ScaleFactor + 25)*y_Scale*  ScaleFactor), Scalar(255, 255, 0), 1, 8);  //画出拉出值为0 的垂直刻度线
			...
			{
				itoa(i, str, 10);
				putText(resultMat, str, Point((int)(t - 7 * ScaleFactor)*x_Scale, (int)(240 - BowNet_Result.pixel_BowHeight / ScaleFactor + 25)*y_Scale*  ScaleFactor), FONT_HERSHEY_PLAIN, 0.5*ScaleFactor, Scalar(255, 0, 255), 1, 8);
				line(resultMat, Point((int)t*x_Scale, (int)(240 - BowNet_Result.pixel_BowHeight / ScaleFactor + 5)*y_Scale*  ScaleFactor), Point((int)t*x_Scale, (int)(240 - BowNet_Result.pixel_BowHeight / ScaleFactor + 15)*y_Scale*  ScaleFactor), Scalar(255, 255, 0), 1, 8);  //画出拉出值为正的刻度线
			}

			if ((recResult.cross_Point.x != 0) && (recResult.cross_Point.y != 0))
			{
				circle(resultMat, Point(BowNet_Result.crossPx*x_Scale*  ScaleFactor, BowNet_Result.crossPy*y_Scale*  ScaleFactor), 4 * ScaleFactor, CV_RGB(255, 255, 0), 2, 8, 0); //标记出交点
			}
		...
		for (int i = BottomBowHeightScale,k =0; i < TopBowHeightScale;i += 100,k++)  //画导高值刻度线
		{
			...
			itoa(i, str, 10);
			putText(resultMat, str, Point(1, (t + 3)*y_Scale), FONT_HERSHEY_PLAIN, 0.5*ScaleFactor, Scalar(255, 255, 0), 1, 8);
			line(resultMat, Point(20 * ScaleFactor, t*y_Scale), Point(30 * ScaleFactor, t*y_Scale), Scalar(0, 255, 0), 1, 8);
		}
		...
		{
			//bowRange.x = (int)(BowNet_Result.midPx - 120)*x_Scale*  ScaleFactor;
			//bowRange.y = (int)(240 - BowNet_Result.pixel_BowHeight - 5)*y_Scale*  ScaleFactor;
			...
			putTextZH(resultMat, c_dg, Point(resultMat.cols * 17 / 20, resultMat.rows / 18), CV_RGB(255, 0, 255), 8 * x_Scale* ScaleFactor, "楷体");
			putTextZH(resultMat, c_sg, Point(resultMat.cols * 17 / 20, resultMat.rows * 2 / 18), CV_RGB(255, 0, 255), 8 * x_Scale* ScaleFactor, "楷体");
			putTextZH(resultMat, c_zh, Point(resultMat.cols * 17 / 20, resultMat.rows * 17 / 18), CV_RGB(255, 0, 255), 8 * x_Scale* ScaleFactor, "楷体");
		}
	}
